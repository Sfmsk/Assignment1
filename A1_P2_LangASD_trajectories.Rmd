...
However, since the focus of the experiment is that of language development an interesting variable is child MLU. From the model we can tell that the groups are well-balanced. That is, the MLU from the first visit is not significantly different across the two groups. This tells us that children with ASD are approximately one year behind language wise from TD children. 
...
memodel = lmer(CHI_MLU ~ 1 + Diagnosis + VISIT + (1+VISIT|SUBJ), data = Data1, REML = FALSE)
summary(memodel)
...
null = lmer(CHI_MLU ~ 1 + VISIT + (1+VISIT|SUBJ), data = Data1, REML = FALSE)
