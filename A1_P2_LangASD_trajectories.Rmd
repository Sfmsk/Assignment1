...
Most of the variables were well-balanced in accordance with diagnosis, however, when making the model where Age is predicted by Diagnosis, we saw that the mean age of children with ASD was significantly different from the mean age of children who are typically developing. On average children with ASD were 12.67 months older than TD children. 

However, since the focus of the experiment is that of language development an interesting variable is child MLU. From the model we can tell that the groups are well-balanced. That is, the MLU from the first visit is not significantly different across the two groups. 
...
Time had a significantly positive effect on child MLU, (ß = 0.234, se = 0.245, t-value = 9.516, p < .05)

Diagnosis did not significantly affect child MLU, (X2(1) = 2.018, p = .156). 
...
Time had a significantly positive effect on parental MLU, (ß = 0.12, se = 0.018, t-value = 6.595, p < .05)

Diagnosis also had a significantly positive effect on parental MLU, (ß = 0.502, se = 0.113, t-value = 4.429, p < .05)

Diagnosis affected parental MLU, (X2(1) = 16.728, p < .05), increasing MLU by 0.502 ± 0.133 se. 
...
We have decided to add 3 more fixed effects to the model and remove one - diagnosis. We decided to keep visit, since it explained a lot in the previous models. Diagnosis, however, did not. Then we decided to add verbalIQ. It seems important because it indicates the linguistic skills of the first visit of the child. We chose to add MOT_MLU because we expect the parental MLU to have an influence on the child's MLU. That is, a larger parental MLU will likely lead to a larger child MLU. Lastly, we added types_CHI, which is the use of unique words of the child. We expect that children who use many unique words will be more likely to have better linguistics skills and therefore longer utterances. 
...
model1 = glm(Gender ~ Diagnosis, family = binomial, Data1)
summary(model1)

model2 = lm(Age ~ Diagnosis, Data1)
summary(model2)

subset = subset(Data1, VISIT == "1", select = c(Diagnosis, CHI_MLU, verbalIQ1, nonVerbalIQ1))

model3 = lm(CHI_MLU ~ Diagnosis, subset)
summary(model3)

model4 = lm(verbalIQ1 ~ Diagnosis, subset)
summary(model4)

model5 = lm(nonVerbalIQ1 ~ Diagnosis, subset)
summary(model5)

model6 = glm(Diagnosis ~ Ethnicity, family = binomial, Data1)
summary(model6)



ggplot(Data1, aes(Diagnosis, Age)) +
  geom_violin()

ggplot(subset, aes(Diagnosis, CHI_MLU)) +
  geom_violin()

ggplot(subset, aes(Diagnosis, verbalIQ1)) +
  geom_violin()

ggplot(subset, aes(Diagnosis, nonVerbalIQ1)) +
  geom_violin()
...
memodel = lmer(CHI_MLU ~ 1 + VISIT + Diagnosis + (1+VISIT|SUBJ), REML = FALSE, Data1)
summary(memodel)
...
null1 = lmer(CHI_MLU ~ 1 + VISIT + (1+VISIT|SUBJ), data = Data1, REML = FALSE)

anova(null1, memodel)

r.squaredGLMM(memodel)
...
Data1$VISIT = as.factor(Data1$VISIT)

contrasts(Data1$VISIT) = contr.poly(6)
model = aov(CHI_MLU ~ 1 + Diagnosis + VISIT + (1+VISIT|SUBJ), Data1, REML = FALSE)
summary.lm(model)
...
Data1$VISIT = as.integer(Data1$VISIT)

memodel2 = lmer(MOT_MLU ~ 1 + VISIT + Diagnosis + (1+VISIT|SUBJ), REML = FALSE, Data1)
summary(memodel2)

plot(memodel2)

null2 = lmer(MOT_MLU ~ 1 + VISIT + (1+VISIT|SUBJ), data = Data1, REML = FALSE)

anova(null2, memodel2)

r.squaredGLMM(memodel2)
...
memodel3 = lmer(CHI_MLU ~ 1 + VISIT + verbalIQ1 + MOT_MLU + types_CHI + (1+VISIT|SUBJ), REML = FALSE, Data1)
summary(memodel3)

r.squaredGLMM(memodel3)
